...
public class TaskListController {
    ...
    @Operation(summary = "Rename task list")
    @PatchMapping("/{taskListId}/name")
    public TaskListResponse rename(@PathVariable UUID taskListId, @Valid @RequestBody TaskListRenameRequest request) {
        return taskListService.rename(taskListId, request);
    }

    @Operation(summary = "Archive task list")
    @DeleteMapping("/{taskListId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void delete(@PathVariable UUID taskListId) {
